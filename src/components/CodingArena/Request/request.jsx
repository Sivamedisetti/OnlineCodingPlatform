...
const Request = () => {
  ...
  let columns = [
    ...
    {
      name: "Difficulty",
      selector: (row) => row.Topic_difficulty,
      sortable: true,
      width: "200px",
    },
    {
      ...
      cell: (row) => (
        <div style={{ display: "flex" }}>
          <p
            ...
            onClick={() => change_Status("Accepted", row._id)}
          >
            Accept
          </p>
          <span>/</span>
          <p
            ...
            onClick={() => change_Status("Rejected", row._id)}
          >
            Reject
          </p>
        </div>
  ...
  function change_Status(status, id) {
    api.put("/update_status", { status, id }).then(() => {
      APICalling();
        ...
        res.data.forEach((ele, key) => {
          ...
          ele.Topic_difficulty = (
            <p
              style={{
                backgroundColor:
                  ele["Topic_difficulty"] === "Easy" ||
                  ele["Topic_difficulty"] === "easy"
                    ? "var(--color-primary-green)"
                    : ele["Topic_difficulty"] === "Medium" ||
                        ele["Topic_difficulty"] === "medium"
                      ? "orange"
                      : "red",
                color: "white",
